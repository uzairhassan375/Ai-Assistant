...
class TaskService {
  ...
  Stream<List<Task>> getCompletedTasksStream({
    ...
    print('🚀 TaskService.getCompletedTasksStream called with userId: $userId, isReminder: $isReminder');
    
    Query query = _firestore
        .collection(_collection)
        .where('userId', isEqualTo: userId)
        .where('isCompleted', isEqualTo: true)
        .where('isArchived', isEqualTo: false); // Don't show archived completed tasks

    if (isReminder != null) {
      query = query.where('isReminder', isEqualTo: isReminder);
    }

    query = query.orderBy('dueDate', descending: true); // Show most recent first

    return query.snapshots().handleError((error) {
      print('🔥 Firestore Stream Error in getCompletedTasksStream: $error');
      ...
      if (error.toString().contains('failed-precondition') || 
        ...
        if (match != null) {
          ...
          print('👉 You need to create a Firestore composite index for this query.');
          print('   Query: userId + isCompleted + isArchived + isReminder + orderBy(dueDate)');
          print('   Go to Firebase Console > Firestore > Indexes to create it.');
        }
      }
      throw error;
      ...
      final allDocs = snapshot.docs;
      print('📊 Raw completed tasks found: ${allDocs.length}');
      ...
      final tasks = allDocs
          .map(
            (doc) {
              try {
                final task = Task.fromMap(doc.id, doc.data() as Map<String, dynamic>);
                print('   ✅ Completed task: "${task.title}" - isCompleted: ${task.isCompleted}, isReminder: ${task.isReminder}');
                return task;
              } catch (e) {
                print('   ❌ Error parsing completed task ${doc.id}: $e');
                return null;
              }
            }
          )
      ...
      print('✅ TaskService.getCompletedTasksStream: Found ${tasks.length} completed tasks for userId: $userId, isReminder: $isReminder');
      return tasks;
    });
  }
